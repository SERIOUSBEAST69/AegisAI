                ...
                <div class="step-layout step-layout-form">
                  ...
                  <form class="login-form" @submit.prevent="handlePrimaryAction" novalidate>
                    <template v-if="flowType === 'login' && activeMode === 'password'">
                      ...
                      <div class="field-group">
                        <label for="login-captcha">验证码（可选）</label>
                        <div class="password-wrap">
                          <input id="login-captcha" v-model.trim="passwordForm.captcha" class="field-input" type="text" maxlength="4" autocomplete="one-time-code" placeholder="留空可跳过" />
...
const accessFlows = [
  { value: 'login', accent: 'RETURNING ACCESS', title: '登录工作台', description: '已有账号、手机号或微信身份时，从这里直接进入。至少保留一步一步的引导，但页面始终只有一张主卡片。' },
...
const currentStepMeta = computed(() => {
  ...
  if (currentStep.value === 2) {
    return {
      ...
      title: '再选择身份建立通道',
      description: '账号密码、手机验证、微信身份分开选择，不和上一屏混在一起。',
    };
  }
...
function getFormValidationError() {
  if (flowType.value === 'login') {
    if (activeMode.value === 'password') {
      ...
      if (passwordForm.captcha && passwordForm.captcha.toUpperCase() !== captchaCode.value) {
        return '验证码错误';
      }
      return '';
    }
